...
func main() {
	...
	webhookServer := webhook.NewServer(webhook.Options{
		TLSOpts: webhookTLSOpts, // use webhook-specific TLS opts (may include certwatcher)
	})

	// Metrics server options (no kube-rbac-proxy).
	metricsServerOptions := metricsserver.Options{
		BindAddress:   metricsAddr,
		SecureServing: secureMetrics,
		TLSOpts:       tlsOpts, // may be extended below if metrics certs provided
	}
	if secureMetrics {
