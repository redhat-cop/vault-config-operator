...
- [ ] Task 1: Update `go.mod` (AC: #1)
  - [ ] 1.1 Change `go 1.22.0` to `go 1.26`
  - [ ] 1.2 Add `toolchain go1.26.4` directive (current latest patch)
  - [ ] 1.3 Run `go mod tidy` to update `go.sum` and resolve any dependency conflicts
  - [ ] 1.4 Verify `go build ./...` succeeds
  - [ ] 1.5 Verify `go vet ./...` passes
  - [ ] 1.6 Run `go test ./...` (unit tests via envtest — may need ENVTEST fix from Story 8.3, but should work with current envtest)
- [ ] Task 2: Update Dockerfile for Go 1.26 + multi-arch (AC: #2, #4)
  - [ ] 2.1 Change `FROM golang:1.22 AS builder` to `FROM --platform=$BUILDPLATFORM golang:1.26 AS builder`
  - [ ] 2.2 Add `ARG TARGETOS` and `ARG TARGETARCH` declarations
  - [ ] 2.3 Replace `RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o manager main.go` with `RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -a -o manager main.go`
  - [ ] 2.4 Verify `podman build .` or `docker build .` succeeds
- [ ] Task 3: Update CI workflows (AC: #3)
  - [ ] 3.1 In `.github/workflows/pr.yaml`: change `GO_VERSION: ~1.22` to `GO_VERSION: ~1.26`
  - [ ] 3.2 In `.github/workflows/push.yaml`: change `GO_VERSION: ~1.22` to `GO_VERSION: ~1.26`
- [ ] Task 4: Update project-context.md (AC: #1)
  - [ ] 4.1 Update `_bmad-output/project-context.md` to reflect Go 1.26 (Language, Container image, and any related references)
- [ ] Task 5: Verify compilation and tests (AC: #1)
  - [ ] 5.1 Run `make fmt && make vet`
  - [ ] 5.2 Run `make test` (unit tests)
  - [ ] 5.3 Verify no new lint warnings from `go vet`
...
Go 1.21+ introduced the `toolchain` directive in `go.mod`. Since we're jumping from `go 1.22.0` (which did not have a `toolchain` line), add `toolchain go1.26.4` to pin the exact patch version used for builds. This ensures reproducible builds and prevents `go` from auto-downloading a different toolchain version.
...
- The `go.sum` changes will be large (hash updates for all dependencies) — this is expected and normal for a Go version bump
