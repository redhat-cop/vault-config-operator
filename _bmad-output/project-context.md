...
- **Strict webhook validation philosophy:** Reject invalid field combinations at admission. Relax a combination only when Vault genuinely supports it universally — not because a single backend or version happens to accept it. Prefer failing closed at the webhook over discovering an unusable spec at reconcile time.
- **Credential key defaulting:** When a type remaps `RootCredentialConfig` keys (e.g. AWS `access_key`/`secret_key`, Okta `api_token`), the defaulter must not overwrite keys the user explicitly set. Inspect the admission request body and default only omitted keys. Empty-string fallback is acceptable when no admission request is present (unit tests). Never treat the inherited `"username"`/`"password"` values as always-overwriteable — those strings are valid explicit keys.
...
- **Critical rule — `toMap()` output normalization:** When Vault transforms a field's format on read (e.g., duration string `"2h"` → integer seconds `7200`, HCL/JSON string → parsed map object), `toMap()` must emit the **Vault-read format** so that `IsEquivalentToDesiredState` compares matching shapes. Use `durationToSeconds()` for duration→seconds conversion (returns `json.Number`). Use `json.Unmarshal` or HCL parsing for string→map conversion. If parsing fails, exclude the field from comparison as a graceful fallback (prevents perpetual drift). This ensures updates propagate correctly through the `CreateOrUpdate` flow while avoiding false drift from format mismatches.
- **Mandatory — duration fields in `toMap()`:** Every duration/TTL field that Vault returns as integer seconds MUST go through `durationToSeconds()` (string specs) or `json.Number(strconv.Itoa(...))` (integer specs). Emitting raw duration strings (`"1h"`) is a latent drift bug. Do not copy older types that still emit strings — those were a defect, now corrected for the auth-engine TTL set. New types (including all Epic 16 auth methods) must use this rule from the first commit.
...
- Any type with custom `IsEquivalentToDesiredState` logic (field deletion, field remapping, alternate map method, field-by-field comparison) **must** have explicit negative tests proving the custom behavior:
  - **Secret-stripping types** (e.g., GitHubSecretEngineConfig deletes `prv_key`, KubernetesSecretEngineConfig deletes `service_account_jwt`, LDAPAuthEngineConfig deletes `bindpass`, QuaySecretEngineConfig deletes `password`): The Vault API never returns write-once secrets on read — these fields are absent or nil in the GET response. The `delete(desiredState, ...)` calls are defense-in-depth, and `filterPayloadToDesiredKeys` already excludes any key not in `desiredState`. Tests should verify that payloads **with or without** the redacted key return `true` (no false drift), since both scenarios are equivalent after filtering. A negative test should instead prove that a **managed** field mismatch still returns `false`.
