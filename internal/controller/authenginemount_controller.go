...
func (r *AuthEngineMountReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	vaultEngineResource := vaultresourcecontroller2.NewVaultEngineResource(&r.ReconcilerBase, instance)
...
func (r *AuthEngineMountReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.AuthEngineMount{}, builder.WithPredicates(vaultresourcecontroller2.NewDefaultPeriodicReconcilePredicate())).
		Named("vaultconfigoperator-authenginemount").
		Complete(r)
}
