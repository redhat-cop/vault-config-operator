...
// LDAPSecretEngineConfigReconciler reconciles a LDAPSecretEngineConfig object
type LDAPSecretEngineConfigReconciler struct {
...
func (r *LDAPSecretEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.LDAPSecretEngineConfig{}
...
func (r *LDAPSecretEngineConfigReconciler) manageReconcileLogic(ctx context.Context, instance client.Object) error {
	...
	// Always write config: bindpass is write-only (Vault never returns it on read),
...
func (r *LDAPSecretEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.LDAPSecretEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			configs, err := r.findApplicableLDAPSEConfigForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable LDAPSecretEngineConfigs for secret", "secret", s.Name)
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			configs, err := r.findApplicableLDAPSEConfigForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable LDAPSecretEngineConfigs for RandomSecret", "randomSecret", rs.Name)
				return []reconcile.Request{}
			}
			for _, config := range configs {
				res = append(res, reconcile.Request{
...
func (r *LDAPSecretEngineConfigReconciler) findApplicableLDAPSEConfigForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.LDAPSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.LDAPSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.LDAPSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of LDAPSecretEngineConfig")
		return nil, err
	}
...
func (r *LDAPSecretEngineConfigReconciler) findApplicableLDAPSEConfigForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.LDAPSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.LDAPSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.LDAPSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
