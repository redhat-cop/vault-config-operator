...
// NomadSecretEngineConfigReconciler reconciles a NomadSecretEngineConfig object
type NomadSecretEngineConfigReconciler struct {
...
func (r *NomadSecretEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.NomadSecretEngineConfig{}
...
func (r *NomadSecretEngineConfigReconciler) manageReconcileLogic(ctx context.Context, instance client.Object) error {
...
func (r *NomadSecretEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.NomadSecretEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			configs, err := r.findApplicableNomadConfigForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable NomadSecretEngineConfigs for secret", "secret", s.Name)
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			configs, err := r.findApplicableNomadConfigForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable NomadSecretEngineConfigs for RandomSecret", "randomSecret", rs.Name)
				return []reconcile.Request{}
			}
			for _, config := range configs {
				res = append(res, reconcile.Request{
...
func (r *NomadSecretEngineConfigReconciler) findApplicableNomadConfigForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.NomadSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.NomadSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.NomadSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of NomadSecretEngineConfig")
		return nil, err
	}
...
func (r *NomadSecretEngineConfigReconciler) findApplicableNomadConfigForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.NomadSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.NomadSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.NomadSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
