...
// CFAuthEngineConfigReconciler reconciles a CFAuthEngineConfig object
type CFAuthEngineConfigReconciler struct {
...
func (r *CFAuthEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.CFAuthEngineConfig{}
...
func (r *CFAuthEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {

	isCFCredentialSecret := predicate.Funcs{
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.CFAuthEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			cfaecs, err := r.findApplicableCFAECForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable CFAuthEngineConfigs for namespace", "namespace", s.Namespace)
				return []reconcile.Request{}
			}
			for _, cfaec := range cfaecs {
				res = append(res, reconcile.Request{
					NamespacedName: types.NamespacedName{
						Name:      cfaec.GetName(),
						Namespace: cfaec.GetNamespace(),
					},
				})
			}
			return res
		}), builder.WithPredicates(isCFCredentialSecret)).
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			cfaecs, err := r.findApplicableCFAECForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable CFAuthEngineConfigs for namespace", "namespace", rs.Namespace)
				return []reconcile.Request{}
			}
			for _, cfaec := range cfaecs {
				res = append(res, reconcile.Request{
...
func (r *CFAuthEngineConfigReconciler) findApplicableCFAECForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.CFAuthEngineConfig, error) {
	result := []redhatcopv1alpha1.CFAuthEngineConfig{}
	vrl := &redhatcopv1alpha1.CFAuthEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of CFAuthEngineConfig")
		return nil, err
	}
	for _, vr := range vrl.Items {
		if vr.Spec.CFCredentials.Secret != nil && vr.Spec.CFCredentials.Secret.Name == secret.Name {
...
func (r *CFAuthEngineConfigReconciler) findApplicableCFAECForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.CFAuthEngineConfig, error) {
	result := []redhatcopv1alpha1.CFAuthEngineConfig{}
	vrl := &redhatcopv1alpha1.CFAuthEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
	...
	for _, vr := range vrl.Items {
		if vr.Spec.CFCredentials.RandomSecret != nil && vr.Spec.CFCredentials.RandomSecret.Name == randomSecret.Name {
			result = append(result, vr)
		}
	}
	return result, nil
