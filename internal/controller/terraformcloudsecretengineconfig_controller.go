...
// TerraformCloudSecretEngineConfigReconciler reconciles a TerraformCloudSecretEngineConfig object
type TerraformCloudSecretEngineConfigReconciler struct {
...
func (r *TerraformCloudSecretEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.TerraformCloudSecretEngineConfig{}
...
func (r *TerraformCloudSecretEngineConfigReconciler) manageReconcileLogic(ctx context.Context, instance client.Object) error {
	...
	// Always write: token is write-only (Vault never returns it on read),
	// so drift detection cannot observe credential rotations.
	vaultEndpoint := vaultutils.NewVaultEndpoint(instance)
...
func (r *TerraformCloudSecretEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.TerraformCloudSecretEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			tfcconfigs, err := r.findApplicableTFCConfigForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable TerraformCloudSecretEngineConfigs for secret", "secret", s.Name)
				return []reconcile.Request{}
			}
			for _, tfcconfig := range tfcconfigs {
				res = append(res, reconcile.Request{
					NamespacedName: types.NamespacedName{
						Name:      tfcconfig.GetName(),
						Namespace: tfcconfig.GetNamespace(),
					},
				})
			}
			return res
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			tfcconfigs, err := r.findApplicableTFCConfigForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable TerraformCloudSecretEngineConfigs for RandomSecret", "randomSecret", rs.Name)
				return []reconcile.Request{}
			}
			for _, tfcconfig := range tfcconfigs {
				res = append(res, reconcile.Request{
...
func (r *TerraformCloudSecretEngineConfigReconciler) findApplicableTFCConfigForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.TerraformCloudSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.TerraformCloudSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.TerraformCloudSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of TerraformCloudSecretEngineConfig")
		return nil, err
	}
	for _, vr := range vrl.Items {
		if vr.Spec.TFCCredentials.Secret != nil && vr.Spec.TFCCredentials.Secret.Name == secret.Name {
...
func (r *TerraformCloudSecretEngineConfigReconciler) findApplicableTFCConfigForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.TerraformCloudSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.TerraformCloudSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.TerraformCloudSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
	...
	for _, vr := range vrl.Items {
		if vr.Spec.TFCCredentials.RandomSecret != nil && vr.Spec.TFCCredentials.RandomSecret.Name == randomSecret.Name {
			result = append(result, vr)
		}
	}
	return result, nil
