...
// ConsulSecretEngineConfigReconciler reconciles a ConsulSecretEngineConfig object
type ConsulSecretEngineConfigReconciler struct {
...
func (r *ConsulSecretEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.ConsulSecretEngineConfig{}
...
func (r *ConsulSecretEngineConfigReconciler) manageReconcileLogic(ctx context.Context, instance client.Object) error {
...
func (r *ConsulSecretEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.ConsulSecretEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			configs, err := r.findApplicableConsulConfigForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable ConsulSecretEngineConfigs for secret", "secret", s.Name)
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			configs, err := r.findApplicableConsulConfigForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable ConsulSecretEngineConfigs for RandomSecret", "randomSecret", rs.Name)
				return []reconcile.Request{}
			}
			for _, config := range configs {
				res = append(res, reconcile.Request{
...
func (r *ConsulSecretEngineConfigReconciler) findApplicableConsulConfigForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.ConsulSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.ConsulSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.ConsulSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of ConsulSecretEngineConfig")
		return nil, err
	}
...
func (r *ConsulSecretEngineConfigReconciler) findApplicableConsulConfigForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.ConsulSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.ConsulSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.ConsulSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
