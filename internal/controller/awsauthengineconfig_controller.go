...
// AWSAuthEngineClientConfigReconciler reconciles a AWSAuthEngineClientConfig object
type AWSAuthEngineClientConfigReconciler struct {
...
func (r *AWSAuthEngineClientConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.AWSAuthEngineClientConfig{}
...
func (r *AWSAuthEngineClientConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.AWSAuthEngineClientConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			configs, err := r.findApplicableAWSAuthConfigsForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable AWSAuthEngineClientConfig for namespace", "namespace", s.Name)
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			configs, err := r.findApplicableAWSAuthConfigsForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable AWSAuthEngineClientConfig for namespace", "namespace", rs.Name)
				return []reconcile.Request{}
			}
			for _, config := range configs {
				res = append(res, reconcile.Request{
...
func (r *AWSAuthEngineClientConfigReconciler) findApplicableAWSAuthConfigsForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.AWSAuthEngineClientConfig, error) {
	result := []redhatcopv1alpha1.AWSAuthEngineClientConfig{}
	vrl := &redhatcopv1alpha1.AWSAuthEngineClientConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of AWSAuthEngineClientConfig")
		return nil, err
	}
	for _, vr := range vrl.Items {
		if vr.Spec.AWSCredentials.Secret != nil && vr.Spec.AWSCredentials.Secret.Name == secret.Name {
...
func (r *AWSAuthEngineClientConfigReconciler) findApplicableAWSAuthConfigsForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.AWSAuthEngineClientConfig, error) {
	result := []redhatcopv1alpha1.AWSAuthEngineClientConfig{}
	vrl := &redhatcopv1alpha1.AWSAuthEngineClientConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
	...
	for _, vr := range vrl.Items {
		if vr.Spec.AWSCredentials.RandomSecret != nil && vr.Spec.AWSCredentials.RandomSecret.Name == randomSecret.Name {
			result = append(result, vr)
		}
	}
	return result, nil
