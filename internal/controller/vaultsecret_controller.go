...
func (r *VaultSecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !ok {
		...
		return vaultresourcecontroller2.ManageOutcome(ctx, r.ReconcilerBase, instance, nil)
	}
	...
	if nextSchedule > 0 {
		return vaultresourcecontroller2.ManageOutcomeWithRequeue(ctx, r.ReconcilerBase, instance, err, nextSchedule)
	} else {
		return vaultresourcecontroller2.ManageOutcomeWithRequeue(ctx, r.ReconcilerBase, instance, err, time.Second)
	}
...
func (r *VaultSecretReconciler) formatK8sSecret(instance *redhatcopv1alpha1.VaultSecret, data interface{}) (*corev1.Secret, error) {
	...
	for k, v := range instance.Spec.TemplatizedK8sSecret.StringData {
		...
		tpl, err := template.New("").Funcs(vaultresourcecontroller2.AdvancedTemplateFuncMap(r.GetRestConfig(), r.Log)).Parse(v)
		if err != nil {
...
func (r *VaultSecretReconciler) shouldSync(ctx context.Context, instance *redhatcopv1alpha1.VaultSecret) (bool, error) {
	...
	if err != nil {
		...
		if !vaultresourcecontroller2.IsOwner(instance, secret) {
