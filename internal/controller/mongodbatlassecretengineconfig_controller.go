...
// MongoDBAtlasSecretEngineConfigReconciler reconciles a MongoDBAtlasSecretEngineConfig object
type MongoDBAtlasSecretEngineConfigReconciler struct {
...
func (r *MongoDBAtlasSecretEngineConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	instance := &redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig{}
...
func (r *MongoDBAtlasSecretEngineConfigReconciler) manageReconcileLogic(ctx context.Context, instance client.Object) error {
...
func (r *MongoDBAtlasSecretEngineConfigReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig{}, builder.WithPredicates(vaultresourcecontroller.NewDefaultPeriodicReconcilePredicate())).
		Watches(&corev1.Secret{
			...
			s := a.(*corev1.Secret)
			configs, err := r.findApplicableMongoDBAtlasConfigForSecret(ctx, s)
			if err != nil {
				r.Log.Error(err, "unable to find applicable MongoDBAtlasSecretEngineConfigs for secret", "secret", s.Name)
		...
		Watches(&redhatcopv1alpha1.RandomSecret{
			...
			configs, err := r.findApplicableMongoDBAtlasConfigForRandomSecret(ctx, rs)
			if err != nil {
				r.Log.Error(err, "unable to find applicable MongoDBAtlasSecretEngineConfigs for RandomSecret", "randomSecret", rs.Name)
				return []reconcile.Request{}
			}
			for _, config := range configs {
				res = append(res, reconcile.Request{
...
func (r *MongoDBAtlasSecretEngineConfigReconciler) findApplicableMongoDBAtlasConfigForSecret(ctx context.Context, secret *corev1.Secret) ([]redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.MongoDBAtlasSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
		Namespace: secret.Namespace,
	})
	if err != nil {
		r.Log.Error(err, "unable to retrieve the list of MongoDBAtlasSecretEngineConfig")
		return nil, err
	}
...
func (r *MongoDBAtlasSecretEngineConfigReconciler) findApplicableMongoDBAtlasConfigForRandomSecret(ctx context.Context, randomSecret *redhatcopv1alpha1.RandomSecret) ([]redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig, error) {
	result := []redhatcopv1alpha1.MongoDBAtlasSecretEngineConfig{}
	vrl := &redhatcopv1alpha1.MongoDBAtlasSecretEngineConfigList{}
	err := r.GetClient().List(ctx, vrl, &client.ListOptions{
